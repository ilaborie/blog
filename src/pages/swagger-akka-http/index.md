...
To automatically generate the server side implementation, it usually doesn't produce fully detailed implementation, but it generates interfaces (in Scala, it would be traits) which you can extend to hook up detailed behaivor in a later phase.
...
- or you already have the server side implementation, and want to start using Swagger for better API management, and leverage Swagger toolset
...
In both cases, the automated generation tool is very important, to make sure there is only one source of truth and the other generated side is compliant with the original. If the automated tool is not avaialble no matter whether it is for the top-down or bottom-up approach, you need a lot more effort to make sure the server implementation is compliant with the API. 

Probably you can leverage the swagger specification file to generate automated test cases, but it is still much better to have an auto-gen tool for either the spec file or the server implementation.
...
To see how it works, I did some experiment - I tried to generate the same specification as [the official Swagger PetStore sample](http://petstore.swagger.io/) from the server `@Annotation`. Looking at the sample, the top section of the spec can be generated by the following `SwaggerDocService` implementation.  
...
To be honest, I felt like writing up the specification directly in json or yaml would have been easier, however, there is no top-down approach tool exists for Akka HTTP to generate the Akka HTTP server code from the spec file at the moment..

Another thing I expected to just work fine earlier, but not avaialble at the moment is that those Swagger `@Annotation`'s compilation errors do not detect almost any type of specification (`@Annotation`) and implementation (e.g. parameters and their types) mismatch. So again, it's all your manual effort to make sure your server implementation is compliant with the specification.
...
Yes, this is another interesting point, but the conventional way in the JVM world to generate the swagger spec from server code's `@Annotation`, even in Java frameworks like [Spring](https://springfox.github.io/springfox/docs/current/#springfox-swagger-ui), is to run the web server and access to the rendered page. 

This is NOT very Continuous-Integration friendly as your build tool won't usually bring up the web server, so it is difficult to put the generated specification file in the build pipeline, like automated testing. 
...
you don't need to add `@Annotation` to the case classes, but Swagger UI nicely generate this much of model definition. Note that the `Pet` case class has some parameters in other case classes, and that is correctly and automatically handled by swagger spec generation.
...
So if you still feel it is beneficial that the endpoint specification (`@Annotation`) being close to the Akka HTTP route implementation, rather than a separate .json/.yaml file, swagger-akka-http is the way to go.
...
![Swagger-UI-curl](./Swagger-UI-curl.png)
